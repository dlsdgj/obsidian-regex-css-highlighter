...
- **备注弹窗文件名同步修复** — 修复重命名笔记后备注弹窗中文件名不更新的问题；`handleFileRenameOrMove` 现在会更新所有规则（当前文件、全局、其他文件）中的 `links[].filePath` 并持久化到磁盘
...
- **分组按钮默认样式** — 分组按钮默认样式从蓝底白字改为白底黑字；自定义样式现在可正确覆盖默认样式
- **单显标签元素修复** — 单显标签从 `<button>` 改为 `<h4>` 元素，修复自定义样式（渐变背景、自定义颜色）无法正确应用的问题
- **单显分组激活状态持久化** — 单显分组的激活状态现在会被保存，重新打开主面板时自动恢复
...
- **手机版右键菜单修复** — 修复长按样式按钮选项卡死、点击无反应、关闭主面板后选项仍显示的问题
- **手机版移除子菜单** — 移除手机端"添加为标题样式"和"移动到分组"子菜单（依赖鼠标悬浮事件，手机无法操作）
- **备注弹窗悬浮延迟** — 新增设置项，控制鼠标悬浮多久后显示备注弹窗，鼠标提前离开则取消显示，防止意外触发
- **添加分组即时刷新** — 修复添加新分组后主面板不显示，需重新打开才显示的问题
...
- **单显分组📌按钮修复** — 单显标签📌按钮改为绝对定位（与常显分组一致），移除overflow:hidden裁切问题
- **主面板暗色背景修复** — 注入!important CSS规则(.rch-transparent-bg)强制覆盖Obsidian默认modal-bg背景，配合MutationObserver三重保障
- **主面板锁定功能完善** — 锁定按钮直接设置modal-bg和modal-container的pointer-events穿透，允许操作编辑器；修复之前无效的CSS class方案
- **锁定焦点抢夺修复** — 锁定时拦截非用户点击触发的focusin事件，将焦点还给编辑器，防止面板抢夺焦点
- **调整大小按钮跟随面板** — 拖动主面板时右下角调整大小按钮实时更新位置
...
- **标题栏固定** — 主面板滚动时标题栏固定在顶部不动
- **滚动条位置修复** — 调整主面板大小时滚动条不再跳到关闭按钮左侧
- **移除显示设置** — 移除"显示"项下"主面板透明度"和"主面板宽度"设置，Ctrl+滚轮和Alt+滚轮快捷键仍可使用
- **分组切换即时生效** — 切换"默认隐藏所有分组"后立即刷新主面板，无需等待下次打开
- **弹窗层级修复** — CSS编辑器和备注编辑弹窗不再被主面板遮挡
- **取消按钮优化** — CSS编辑器点击"取消"不再触发不必要的UI刷新
- **折叠展开性能** — 展开/折叠操作不再等待文件保存，消除CSS编辑器关闭后1.7秒延迟
- **主面板打开速度** — 延迟渲染规则/设置区域，移除强制重排，批量字体修复，加快面板打开速度
...
- **规则转换合并** — 全局/局部规则转换时，若目标已存在同regex规则，cssClass覆盖、remark和links合并，不再阻止转换
- **弹窗规则来源标记** — 备注弹窗右上角添加l/g(局部/全局)按钮，始终置顶显示，点击可转换规则来源
- **备注弹窗国际化** — 翻译备注弹窗中硬编码的中文文本（双击编辑备注、无搜索词、无标题、复制/打开/删除备注），支持英文模式
- **更新备注演示GIF** — 替换为新的备注功能演示动画
