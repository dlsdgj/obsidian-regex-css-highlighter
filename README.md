...
D:\Documents\Obsidian Vault\.obsidian\plugins\Regex-Css-Highlighter\main.js
...
<details>
<summary> floating notes</summary>
...
<details>
<summary>v2.0.7 (2026-08-04)</summary>
...
- **First Install Collapsed Note / 首装折叠笔记** — Intro floating note appears in collapsed/thumbnail state on first install / 首次安装介绍悬浮笔记以折叠/缩略状态显示
...
- **Preset Prompt i18n / 预设提示词国际化** — Preset prompts auto-translate on language switch; editing a preset converts it to custom / 预设提示词语言切换时自动翻译；编辑后变为自定义
- **New Install Defaults / 新装默认值** — showInfoSection defaults to true; floating notes include consultation info preset (hidden) / 显示Info板块默认开启；悬浮笔记包含咨询信息预设（隐藏）
...
- **Tooltip Disabled State / Tooltip禁用状态** — "Add as Global/File Rule" options grayed out when no text selected / 未选中文字时"添加为全局/文件规则"选项灰色不可点击
- **Action Buttons Redesign / 操作按钮重设计** — c/i/g/l buttons: circular→pill on hover, purple accent (#7c6cf0) matching floating ball / 圆形→悬停药丸形，紫色accent呼应悬浮球
- **Right-Side Expansion / 右侧展开方向** — Buttons expand text leftward when floating ball is on right / 悬浮球右侧时文字向左展开
- **g/l Toggle Capsule / g/l胶囊切换** — Capsule toggle with circular sub-items, active item purple-highlighted / 胶囊toggle圆形子项，激活项紫底

</details>

<details>
<summary>v2.0.1.3 (2026-08-01)</summary>

- **EditingChip Click Opens CSS Editor / 样式chip点击打开CSS编辑器** — Click editingChip opens CSSEditorModal instead of canceling / 点击editingChip打开CSS编辑器
- **Switch Style No Collapse / 切换样式不再折叠** — Only opens style chip if not already pinned / 仅未展开时才点击styleChip
- **Style Button Hover Tooltip / 样式按钮悬停提示** — Interactive tooltip with primary + expandable secondary options / 交互式tooltip含主要+可展开次要选项
- **Remove Update Rule Button / 移除更新规则按钮** — No longer needed in input area / 输入框中不再需要
- **EditingChip Hidden for Non-Rule Text / 非规则文本隐藏editingChip** — Only shows when input matches editing rule regex / 仅输入匹配编辑规则时显示
- **Global Rule Duplicate Prevention / 全局规则重复防护** — Shows notice instead of adding duplicate file rule / 提示而非添加重复文件规则
- **Dynamic Rule Existence Check / 动态规则存在检查** — Dynamic check instead of stale closure variables / 动态检查而非过期闭包变量
- **Highlight Dedup Safety Net / 高亮去重安全网** — Deduplicates allRules by regex+cssClass / 按regex+cssClass去重
- **Sentence Rule Global Guard / 长句全局规则防护** — addGlobalRule rejects regex exceeding sentence threshold / 拒绝超过句子长度阈值的regex
- **addGlobalRule Returns Boolean / 返回布尔值** — Callers show success notice only on true / 调用方仅true时显示成功提示
- **Remove Duplicate Middle-Click / 移除重复中键处理器** — Keeps auxclick only, eliminates double Notice / 仅保留auxclick，消除双重Notice
- **Direct push→addGlobalRule / 直接push改为addGlobalRule** — Consistent sentence rule checking / 统一长句检查
- **Complex Regex Keyword Highlighting / 复杂正则关键词高亮** — _buildKeywordRanges supports lookbehind/non-capturing groups / 支持lookbehind/非捕获组
- **Click-Show Dblclick Edit Fix / 点击显示双击编辑修复** — Row click excludes remarkSpan / row click排除remarkSpan
- **Remark Display Mode Instant Refresh / 评论显示方式即时刷新** — cloneNode + rebind on mode change / 切换时cloneNode+重新绑定
- **Always Mode Display Fix / 常显模式修复** — Sets display='' instead of silent return / 设置display=''而非静默返回
- **Remark Keyword Matching / 评论关键词匹配** — Keywords in comments show CSS styles and are clickable / 评论中关键词显示样式且可点击
- **Float Button Empty JSON Fix / 悬浮按钮空JSON修复** — Checks empty content before JSON.parse / 解析前检查空内容

</details>

<details>
<summary>v2.0.1.2 (2026-07-29)</summary>

- **Prompt Dialog Fix / prompt()对话框修复** — Replaced prompt() with InputModal in Electron / Electron中用InputModal替代prompt()
- **Collapsed Note Disappear Fix / 折叠笔记消失修复** — renderGlobalNotesArea removes both card and mini / 渲染时同时移除card和mini
- **Drag Auto-Expand Fix / 拖动自动展开修复** — Drag state guard in mini click handler / mini click添加拖动状态守卫
- **Drag Position Save / 拖动位置保存** — Mini drag saves _collapsedPos and persists / mini拖动后保存位置并持久化
- **Window Size Loss Fix / 窗口大小丢失修复** — 5-layer protection for 0px size data / 5层防护防止0px尺寸数据
- **Mobile Chip First-Click Fix / 手机chip首次点击修复** — Capture-phase handler with timestamp guard / capture-phase handler加时间戳守卫
- **Mobile Settings Bar Drag / 手机设置栏拖动** — Touch drag support on settings bar / 设置栏添加触摸拖动
- **Panel Dblclick Scroll Fix / 面板双击跳顶修复** — Removed dblclick handlers that caused DOM rebuild and scrollTop reset / 移除导致DOM重建和scrollTop重置的dblclick handler
- **Related Keyword Chip Click Fix / 关联词chip点击修复** — Changed to openKeywordWindow to prevent scroll reset / 改为openKeywordWindow防止滚动重置
- **ScrollTop Preservation / scrollTop保持** — _preservedScrollTop property + setTimeout(0) restore / _preservedScrollTop属性+setTimeout(0)恢复
- **Mini Note Touch/Click Fix / 迷你笔记触摸点击修复** — Unified touch/click with vibrate feedback + double-invocation guard / 统一touch/click加震动反馈和双重调用守卫
- **Floating Ball Toggle as Checkbox / 悬浮球开关改复选框** — Native checkbox replaces chip, avoids touch/click competition / 原生checkbox替代chip避开touch/click竞争
- **Settings Bar Click Exclusion / 设置栏click排除** — Handlers exclude popup/interactive elements / handler排除弹窗和交互元素
...
- **Update Rule Logic Fix / 更新规则逻辑修复** — currentEditingRule only set on rule button click, reset on input change / 仅点击规则按钮时设置，输入变化时重置
- **Color Scheme Save as New / 配色方案保存为新方案** — "Save as New" adds new preset instead of replacing; auto-rename on ID conflict / 新增而非替换，ID冲突自动重命名
- **Related Highlights Settings / 相关高亮设置** — Gear icon with settings popup for both types: font size, line height, uniform keyword style, click-to-show remarks / 两种相关高亮均添加设置：字体大小、行距、关键词样式统一、评论点击显示
- **Related Highlights Renamed / 相关高亮改名** — "Non-Keyword Related Highlights" and "Keyword Related Highlights" / "非关键词相关高亮"和"关键词相关高亮"
- **Uniform Keyword Style / 关键词样式统一** — Unify keyword styles to a single CSS class, fallback to original when empty / 统一关键词样式为指定CSS类名，空值用原样式
- **Click-to-Show Remarks / 高亮评论点击显示** — Hide remark bubbles by default, show on click / 默认隐藏评论气泡，点击显示
- **Chip Hover Mode for Groups / 分组悬停模式** — Keyword group chips respect hover mode setting / 分组chip受悬停模式控制
- **Float Note Width Fix / 悬浮笔记宽度修复** — Removed maxWidth lock; restored max-width:480px / 移除maxWidth锁定，恢复max-width:480px
- **Panel Card Height Fix / 面板笔记高度修复** — Removed _panelCardHeight mechanism, height auto-adapts / 移除_panelCardHeight，高度自适应
- **Dblclick Edit Fix / 双击编辑修复** — Removed span[style] guard blocking dblclick / 移除span[style]守卫，双击可编辑
- **chipBar Position Fix / chipBar位置修复** — Fixed chipBar moved out of chipContentSection / 修复chipBar被移出chipContentSection
- **Keyword Click Fix / 关键词点击修复** — Keywords in related highlights open independent window / 关键词点击打开独立窗口
- **saveSettings Error Fix / saveSettings错误修复** — Fixed saveSettings is not a function / 修复saveSettings未定义
...
- **Mobile Floating Note Toolbar / 手机悬浮笔记操作栏** — Toolbar hidden by default on mobile, tap to toggle / 手机端操作栏默认隐藏，点击切换显示
- **Collapsed Note Drag Fix / 折叠笔记拖动修复** — Desktop: drag mini no longer expands on release; collapsed state persisted across re-renders and restarts / 桌面端拖动mini不再展开；折叠状态持久化
- **Mobile History Keywords Always Visible / 手机端历史关键词常显** — History section always visible on mobile; clear only clears chips; mobile clicks add to history / 历史区块常显，清空仅清chip，手机端点击添加历史
- **Mobile Del Button / 手机端删除按钮** — Added "del" button below c/i/g/l near floating ball for quick rule deletion / 悬浮球旁添加del按钮快速删除规则
- **Mobile Main Panel Scroll / 手机端主面板滚动** — Content area allows vertical scroll (pan-y) / 内容区域允许垂直滚动
- **Mobile Main Panel Drag / 手机端主面板拖动** — Settings bar bottom supports touch drag (floating panel only); resize handle follows / 设置栏底部支持拖动；resize handle跟随
- **Mobile Sidebar Match Fix / 手机端侧边栏匹配修复** — Closing and reopening main panel no longer breaks keyword match display / 关闭主面板后关键词匹配正常显示
- **Floating Note Edit Style / 悬浮笔记编辑样式** — Removed border/radius from edit textarea; fills window size / 编辑框去掉边框圆角，自适应窗口
- **Related Highlights for Rule Keywords / 规则关键词相关高亮** — Keywords with matching rules now show related highlights and backlinks / 有规则的关键词也显示相关高亮和反向引用
...
<details>
<summary>v1.9.9.17.5 (2026-07-26)</summary>
...
<details>
<summary>v1.9.9.17.3 (2026-07-26)</summary>

- **Related Highlight Keyword Style Fix / 相关高亮关键词样式修复** — Fixed keyword CSS style applied to entire entry instead of only matched keyword segments / 修复关键词样式被应用到整段高亮而非仅匹配片段
- **Related Highlight Unified Background / 相关高亮统一背景** — Added consistent light yellow background to all related highlight entries / 所有相关高亮条目统一添加淡黄色背景
- **Mobile Panel Resize Fix / 手机端面板resize修复** — Fixed duplicate resize handles; added touch support to setupModalResizeHandle; enlarged handle on mobile / 修复重复resize按钮，添加touch支持，手机端放大handle
- **Mobile Sidebar Resize Handle Removal / 手机端侧边栏resize移除** — Sidebar no longer shows resize handle / 侧边栏不再显示resize按钮
- **Mobile Mini Note Click Fix / 手机端迷你笔记点击修复** — Fixed touch events blocking click on mini notes; touchend directly restores card / 修复触摸事件阻止迷你笔记点击展开
- **Mobile Inline Note Custom Resize / 手机端内嵌笔记自定义resize** — Custom 20px touch resize handle replaces native CSS resize / 20px自定义触摸resize替代原生CSS resize
- **Mobile Inline Note Collapse Button / 手机端内嵌笔记折叠按钮** — SVG collapse button in toolbar (equivalent to desktop middle-click) / 操作栏添加SVG折叠按钮
- **Panel Lock State Fix / 面板锁定状态修复** — Locked panels can only be closed via close button on all platforms / 锁定面板在所有平台只能通过关闭按钮关闭
- **Middle-Click Link Fix / 中键链接修复** — Fixed middle-click opening new window but not navigating to document / 修复中键链接未导航到文档

</details>

<details>
<summary>v1.9.9.17 (2026-07-25)</summary>

- **AI Graph Button Position Fix / AI关系图按钮位置修复** — Moved ⬡ button back to related keywords block header / 将⬡按钮移回关联词标题栏
- **AI Question Block Collapse / AI提问块折叠** — Added ▽/▷ collapse toggle in AI question block header / 在AI提问块标题栏添加折叠按钮
- **No-Title Chip Removal / 无标题chip移除** — Removed residual no-title chip for AI-only groups / 移除AI分组残留的无标题chip

</details>

<details>
<summary>v1.9.9.16 (2026-07-24)</summary>

- **Masonry Mode Button in Remark Block / 瀑布流按钮移至备注块内** — Moved masonry toggle to inside keyword remarks block; only affects remarks, not related notes/highlights / 瀑布流按钮移至关键词备注块内，仅作用于备注内容
- **Auto Masonry Mode / 自动瀑布流模式** — New remarkMasonryAuto setting: 1 remark → list, multiple → masonry; manual override via button / 自动瀑布流设置，1条→列表，多条→瀑布流
- **Remark & Keywords as Independent Blocks / 备注与关联词独立块** — Remark content wrapped as "关键词备注" block, keyword chips as "关联词" block / 备注内容与关联词分别包裹为独立块
- **AI Relation Graph in Keywords Block / AI关系图移入关联词块** — Graph rendered inside keyword-chips-block, collapsed by default / 关系图在关联词块内部，默认折叠
- **AI Graph Persistence Fix / AI关系图持久化修复** — Fixed _mermaidGraph lost on reload; graph renders after switching keywords / 修复关系图重启后消失，切换关键词后正确渲染
- **AI Graph Max Tokens Setting / AI关系图Max Tokens设置** — New aiGraphMaxTokens setting (default 4096) in AI settings / AI设置面板新增Max Tokens
- **Exclude Related Highlights / 排除相关高亮** — excludedRelatedNotesCategories now also excludes related highlights / 排除分组同时排除相关高亮
- **Scroll Position Restore / 滚动位置恢复** — Panel preserves scroll when toggling masonry mode / 切换瀑布流时保持滚动位置
- **Unified aiBtnContainer Sticky / 统一?+按钮粘底** — Both popup types use position:sticky for ?+ buttons / 两种弹窗统一sticky粘底
- **CodeMirror Import Fix / CodeMirror导入修复** — Added @codemirror/state and @codemirror/view imports / 添加CodeMirror 6依赖导入

</details>

<details>
<summary>v1.9.9.15 (2026-07-24)</summary>

- **Related Keywords Chip Fix / 关联词chip修复** — Sentence threshold no longer applies to related keywords; all display as compact keyword-chips / 句子长度阈值不再作用于关联词，统一渲染为关键词chip
- **Inline Note Action Bar Drag / 内嵌笔记操作栏拖动** — Action bar blank area supports drag to move card / 操作栏空白区域可拖动移动卡片
- **Color Scheme Sync Fix / 配色方案同步修复** — Fixed wrong selectors in `_syncNoteTheme`; panel color changes now sync to inline notes / 修复配色方案切换时内嵌笔记颜色不同步
- **Related Notes → Related Files / 相关笔记改为相关文档** — Renamed with document count displayed after title / 更名并显示文档数量
- **Related File Name Truncation / 相关文档名称截断** — Long names auto-truncate with ellipsis; no horizontal scrollbar / 长文件名截断显示，无横向滚动条
- **Settings Panel Drag / 设置面板拖拽** — Blank area supports drag on desktop and mobile / 空白区域支持拖拽移动
- **Lookahead/Lookbehind Highlight Fix / lookahead规则高亮修复** — `意识(?!到|不)` no longer incorrectly matches "意识到"/"意识不" / lookahead规则不再错误匹配排除文本
- **Regex Rule Chip Display / 正则词chip显示修复** — Regex rules like `bc|d` now correctly show chips when selected / 正则词选中后正确显示chip
- **Chinese Class Name Scheme Fix / 中文类名配色方案修复** — Chinese class names like `.蓝眼泪` now correctly show "Update" button / 中文类名按钮正确显示"更新"
- **Inline Note Scroll Fix / 内嵌笔记滚动修复** — Text area scrolls independently; timestamp fixed at bottom; no double scrollbar in edit / 文本独立滚动，时间戳固定底部，编辑无双滚动条
- **Inline Note Edit Height Fix / 内嵌笔记编辑高度修复** — Card height locked during edit; actual height saved and restored after restart / 编辑时锁定高度，重启后恢复

</details>

<details>
<summary>v1.9.9.14 (2026-07-23)</summary>

- **Sentence Length Threshold / 句子长度阈值** — Added Chinese char count (default 8) and English word count (default 3) settings to distinguish keyword vs sentence rules / 新增中文字数（默认8）和英文词数（默认3）阈值设置，区分关键词与句子规则
- **Remark Keyword Chips / 备注区域关键词chips** — Replaced pipe-separated keywords with clickable chips; default first keyword, click to switch; related highlights follow active chip / 替代"|"分隔为可点击chips，默认显示第一个，点击切换，相关高亮跟随chip
- **SG Sentence Related Highlights / SG句子相关高亮** — Sentence rules shown as related highlights with source file name and jump; merged with OB highlights / 句子规则以相关高亮显示，带源文件名和跳转，与OB高亮合并
- **g/l Button / g/l按钮** — Added g/l button for switching rules between global and file scope / 新增g/l按钮，在全局与文件规则间切换
- **Unified c/i/g/l Buttons / 统一c/i/g/l按钮** — Plain text without border/background, vertical below floating ball; desktop and mobile unified / 纯文本无背景边框，纵向挂在悬浮球下方，桌面与手机统一
- **Desktop Click-to-Show Buttons / 桌面端点击显示按钮** — Click highlighted keyword directly shows c/i/g/l buttons without selecting text / 点击高亮关键词直接显示按钮，无需选中文本
- **Mobile Button Display / 手机端按钮显示** — c/i/g/l buttons shown on mobile when tapping keywords; removed desktop-only restriction / 手机端点击关键词也显示按钮，移除桌面端限制
- **Cross-File Rule Matching / 跨文件规则匹配** — Selecting text searches other files' rules, matching long sentences from different files / 选中文本跨文件搜索规则，匹配其他文件的长句
- **i Button Keyword Support / i按钮关键词支持** — i button passes clicked keyword to interlinear note input instead of requiring selection / i按钮传入关键词文本，不再要求选中文本
- **Button Flash Fix / 按钮闪现修复** — Prevent `selectionchange` from removing buttons immediately after click / 防止selectionchange在点击后立即移除按钮
- **global-notes.json Auto-Repair / global-notes.json自动修复** — JSON parse failure auto-truncates to valid prefix; fixed `onunload` missing `defaultFnGroup` causing corruption / JSON解析失败自动截断修复；修复onunload缺少defaultFnGroup导致损坏

</details>

<details>
<summary>v1.9.9.13 (2026-07-22)</summary>

- **Palette Button Scroll Switch / 调色盘按钮滚轮切换** — Scroll wheel on palette button directly switches color scheme with name tooltip / 调色盘按钮上滚轮直接切换配色方案，显示方案名提示
- **Shift+Scroll Theme Sync Fix / Shift+滚轮切换同步修复** — Fixed box-shadow, backdrop-filter, texture layer not updating on Shift+scroll scheme switch / 修复Shift+滚轮切换时box-shadow、毛玻璃、纹理层未更新
- **Exclude Related Notes Fix / 排除相关笔记功能修复** — Fixed excluded groups being stored but never consumed; excluded group rules now skip related notes / 修复排除分组只存不消费；被排除分组规则不再显示相关笔记
- **Smart Save Button Logic / 保存按钮智能逻辑** — "Update" when only class name or only values changed; "Add as New Scheme" when both differ / 仅改类名或仅改值显示"更新"，都改显示"添加为新方案"
- **i18n Translation / 国际化翻译** — Replaced ~90+ hardcoded Chinese with t(); added 70+ translation keys; removed 25+ Chinese fallbacks / 替换约90+处硬编码中文，新增70+翻译key，移除25+处fallback
- **Cross-View Theme Sync / 跨视图配色同步** — Color scheme changes in panel/inline/popout instantly sync to all other views / 配色切换即时同步到所有视图
- **Inline Right-Click Panel Sync / 内嵌右键面板同步** — Delete/show-hide/move-to-group in inline right-click menu now instantly refreshes panel / 内嵌右键删除/显隐/移动分组即时刷新面板

</details>

<details>
<summary>v1.9.9.12 (2026-07-22)</summary>

- **Panel Remark Double-Click Fix / 面板备注双击编辑修复** — Added container-level dblclick delegation in `renderRemarkContent` so all remark elements respond to double-click editing / 在`renderRemarkContent`中添加事件委托，所有备注元素均可双击编辑
- **Color Scheme Name Tooltip / 配色方案名提示** — Shift+scroll on floating notes shows scheme id as temporary tooltip; inline cards + popout windows / 悬浮笔记Shift+滚轮切换配色时显示方案名；内嵌+独立窗口
- **Inline Note Tooltip Position Fix / 内嵌笔记tooltip定位修复** — Tooltip uses `position:fixed` on `document.body` to prevent changing card position which caused window to disappear / tooltip改为fixed定位挂载body，避免误改卡片position导致窗口消失
- **Inline Note Tooltip Cleanup / 内嵌笔记tooltip清理** — Tooltip stored on `card._schemeTip` prevents duplicates on continuous scroll; `card.remove()` auto-cleans / tip存储在card属性避免重复创建；关闭时自动清理

</details>

<details>
<summary>v1.9.9.11 (2026-07-22)</summary>

- **Custom Schemes Migration / 自定义方案迁移** — Moved `customColorSchemes` from `global-notes.json` to `color-schemes.json`; old `{bg,text}` format converted to CSS rules / 自定义配色从`global-notes.json`迁移到`color-schemes.json`；旧格式转为CSS规则
- **Smart Save Button / 智能保存按钮** — Hidden when no changes; "更新" for existing scheme edits, "保存为新方案" for new schemes; class name changes update correctly / 无变化隐藏；编辑显示"更新"，新建显示"保存为新方案"；类名变更正确更新
- **Unified Color Schemes / 统一配色方案** — New notes cycle `colorSchemePresets`; removed hardcoded theme arrays; all themes derived via `schemeToTheme` / 新建笔记循环配色预设；移除硬编码主题；统一用`schemeToTheme`推导
- **Close Button Enhancement / 关闭按钮增强** — Tooltip "单击关闭/中键点击删除"; middle-click deletes note; inline + popout / tooltip提示操作；中键删除笔记；内嵌+独立窗口
- **Mobile Fixes / 手机端修复** — Color scheme popup touch-draggable; inline display button visible on mobile / 配色弹窗触摸拖动；手机端显示内嵌按钮
- **Related Keywords Per-Section / 相关词按分组显示** — Chips render per-rule before 3 buttons instead of merged at bottom / 相关词按关键词分组渲染在3按钮上方
- **Remove + Button / 移除+按钮** — Removed "+" from color scheme popup header / 移除配色弹窗+按钮
- **Bug Fixes / 错误修复** — Fixed `inputValue` undefined, `insertBefore` error, middle-click closing panel / 修复未定义变量、insertBefore错误、中键关闭面板

</details>

<details>
<summary>v1.9.9.10 (2026-07-21)</summary>

- **Color Scheme CSS Format / 配色方案CSS格式** — Scheme data refactored to standard CSS rules (`.scheme-id { background:...; color:...; }`); id from class name; no class = invalid / 配色方案数据重构为标准CSS规则；id从类名提取；无类名无效
- **Remove Scheme Grouping / 移除分组** — Removed `cat` field and grouped rendering; flat 5-column grid / 移除cat字段和分组渲染；平铺5列网格
- **Trim Class Names / 修剪类名** — `.sg-float-vintage-01` → `.vintage-01` for cleaner editing / 类名修剪，更易编辑
- **Editable Scheme Code / 可编辑方案代码** — Shared textarea replaces read-only `<pre>`; real-time preview on edit; clicking swatch shows CSS / 共用textarea替代只读pre；编辑实时预览；点击色卡显示CSS
- **Add Scheme (+) Button / 添加方案+按钮** — "+" clears textarea for new input; "Save" parses CSS and adds to grid / "+"清空textarea；"保存"解析CSS并添加到网格
- **Middle-Click Delete / 中键删除** — Middle-click swatch to delete scheme; confirmation with "don't ask again" / 中键点击色卡删除方案；确认弹窗含不再询问选项
- **Swatch Selection Fix / 色卡选中修复** — Variable-based selection tracking replaces unreliable style matching / 变量追踪替代不可靠的style属性匹配
- **Popout Palette Removed / 移除独立窗口配色按钮** — Non-functional palette button removed from popout titlebar / 移除不工作的独立窗口配色按钮
- **Popout Texture z-index Fix / 独立窗口纹理修复** — Texture z-index -1→1; content z-index:2 for interaction / 纹理z-index修复；内容层z-index:2确保可交互
- **Legacy Format Compat / 旧格式兼容** — Handles both CSS and legacy `{bg,text}` formats / 兼容CSS和旧{bg,text}格式
- **Custom Schemes Migration / 自定义方案迁移** — 6 existing custom schemes auto-migrated to CSS format / 6个自定义方案自动迁移为CSS格式

</details>

<details>
<summary>v1.9.9.9 (2026-07-20)</summary>

- **SVG Action Buttons / SVG操作按钮** — Replaced emoji with SVG vector icons for inline/panel/popout notes; 24px touch targets; hover highlight; auto-adapts icon color to theme / emoji替换为SVG矢量图标；24px点击区域；悬浮高亮；颜色自适应主题
- **Inline Note Action Bar / 内嵌笔记操作栏** — Top floating bar with semi-transparent background+blur; hidden during editing; double-click anywhere to edit; min-height 80px / 顶部悬浮栏，半透明+模糊；编辑时隐藏；任意位置双击编辑；最小高度80px
- **Panel Note Action Bar / 面板笔记操作栏** — Same SVG style; theme-adaptive colors; timestamp at card bottom / 同样式SVG；颜色自适应；卡片底部时间戳
- **Popout Window Buttons / 独立窗口按钮** — SVG icons in titlebar; always visible with theme-color background; no blur for clean drag area / 标题栏SVG图标；始终可见主题色背景；无模糊保持拖动清晰
- **Default Group Display / 默认分组显示** — Right-click any group chip to set as default; persisted via `defaultFnGroup` / 右键分组chip设为默认；通过`defaultFnGroup`持久化
- **Dynamic Masonry Columns / 动态瀑布流列数** — Column count adjusts by panel width (1 per 160px); ResizeObserver real-time update / 列数随面板宽度调整（每160px一列）；ResizeObserver实时更新
- **JSON Data Safety / JSON数据安全** — Fixed double-closing-brace corruption; auto-restore from backup / 修复双重闭合花括号损坏；自动从备份恢复

</details>

<details>
<summary>v1.9.9.8 (2026-07-20)</summary>

- **Floating Note Data Safety / 悬浮笔记数据安全** — Sync save on unload, snapshot-based write to prevent race condition, backup to `.bak.json` before write, auto-restore from backup when main file empty / 卸载时同步保存，快照防竞态覆盖，写入前备份，主文件为空自动恢复
- **Floating Note Inline Display / 悬浮笔记内嵌显示** — Auto-adjust height to show full text; min-width 200px, max-width 480px; scrollable overflow; timestamp hidden during editing / 高度自适应全文；最小200px最大480px；超出可滚动；编辑时隐藏时间戳
- **Floating Note Preview / 悬浮笔记预览** — Plain text replaces Markdown rendering to eliminate paragraph margins and properly display blank lines / 纯文本替代Markdown渲染，消除段落边距，空行正常显示
- **Floating Note Edit Height / 悬浮笔记编辑高度** — Edit textarea auto-resizes by line count with 80px minimum, fixing single-line issue after restart / 根据行数自适应高度（最低80px），修复重启后编辑仅一行
- **Panel Floating Notes Layout / 面板悬浮笔记布局** — 3-column masonry instead of fixed 2-column; action buttons (📌📋) on hover only; added copy button / 三栏瀑布流；按钮悬浮显示；添加复制按钮

</details>

<details>
<summary>v1.9.9.7 (2026-07-20)</summary>

- **Floating Note Inline Display / 悬浮笔记内嵌显示** — Inline notes support Ctrl/Shift/Alt+scroll adjustments (opacity/theme/font size); edit area expands to card edges; cursor placed at text end on edit / 内嵌笔记支持Ctrl/Shift/Alt+滚轮调整；编辑区扩展到卡片边缘；编辑时光标放在末尾
- **Floating Note Popout Window / 悬浮笔记独立窗口** — Always-on-top without parent window; close hides instead of deletes; back button returns to inline with preserved size; textarea overlay replaces contentEditable for reliable re-editing / 始终置顶无父窗口；关闭隐藏而非删除；返回按钮保持大小切回内嵌；textarea替代contentEditable修复再次编辑
- **Floating Note Themes / 悬浮笔记配色** — 5 new color schemes; increased background opacity; inline uses same theme as popout; custom color editor in right-click menu (replaces unsupported prompt()) / 新增5组配色；提高不透明度；内嵌与独立窗口同配色；右键自定义配色编辑器
- **Floating Note Pin/Copy Buttons / 悬浮笔记📌📋按钮** — Panel 📌 icon: left-click inline, middle-click popout; inline 📌 switches to popout; copy button (📋) on hover / 面板📌左键内嵌中键独立；内嵌📌切换独立；悬浮复制按钮
- **Floating Note Timestamp & Markdown / 悬浮笔记时间戳与Markdown** — Creation timestamp at bottom; inline notes render Markdown via Obsidian's MarkdownRenderer / 底部创建时间戳；内嵌笔记渲染Markdown
- **Floating Note Groups / 悬浮笔记分组** — Note grouping in panel with default archive group; add/rename/delete groups; "Move to group" in right-click; rounded chip style / 面板分组功能；默认归档组；右键移动到分组；圆角chip
- **Panel Right-Click Fix / 面板右键修复** — Right-clicking under chips pins to prevent auto-hide; mouseleave/mouseout excludes .menu targets / 右键时固定chip防隐藏；排除.menu目标
- **New Note Defaults / 新笔记默认值** — 180×140px default size; cycling color schemes / 默认180×140px；循环配色

</details>

<details>
<summary>v1.9.9.6 (2026-07-20)</summary>

- **Floating Notes Chip Close Fix / 悬浮笔记Chip关闭修复** — Moved fnContainer from inside chipBar to after chipBar, fixing mouseout being intercepted when leaving from bottom / 将fnContainer从chipBar内部移到外部，修复从下方离开时不自动关闭
- **Floating Notes Chip Position Fix / 悬浮笔记Chip位置修复** — Added position correction in updateDisplay to keep fnContainer below chipBar after DOM rebuilds / 在updateDisplay中添加位置修正，确保DOM重建后fnContainer紧跟chipBar

</details>

<details>
<summary>v1.9.9.5 (2026-07-20)</summary>

- **Keyword Bar Position & Visibility / 关键词栏位置与可见性** — keywordBar inside chipBar with `flex-basis:100%`; only visible when style chip active / 移入chipBar内部，仅在style chip激活时显示
- **Chip Bar Background Removed / 移除背景** — Removed background from chipBar, keywordBar, rules section / 移除chipBar等背景色
- **Chip Event Delegation Fix / 事件委托修复** — Guard chipBar internal events from triggering `scheduleHide()` / 排除chipBar内部事件防止误触收起
- **Chip mouseleave Fix / 离开事件修复** — `mouseleave` checks `relatedTarget` to avoid hiding when mouse stays in chipBar / 检查relatedTarget防止chipBar内移动时收起

</details>

<details>
<summary>v1.9.9.4 (2026-07-20)</summary>

- **Keyword Switch Bar / 关键词切换栏** — Added keyword button bar below CSS chip bar when multiple rules match; default shows exact-match keyword's styles, click to switch / 多规则匹配时在CSS chip bar下方显示关键词切换按钮，默认显示精确匹配样式，点击切换其他关键词
- **Current Editing Rule Priority / 当前编辑规则优先级** — `currentEditingRule` prioritizes exact match over first-in-array, then longest match / 优先精确匹配，其次最长匹配
- **Style Click State Update / 样式点击状态更新** — After clicking a style, `currentEditingRule` and `highlightMatchingRuleButtons` are updated in all branches, fixing file rule group not appearing / 样式点击后更新状态，修复全局规则点击其他样式后文件规则分组不显示
- **Chip Event Delegation / Chip事件委托** — Changed to event delegation via `mouseover`/`mouseout` + `closest(selector)`, fixing collapsed groups after DOM rebuild / 改用事件委托，修复DOM重建后展开分组收起

</details>

<details>
<summary>v1.9.9.3 (2026-07-20)</summary>

- **Regex Pipe Split Fix / 正则管道符分割修复** — Added `_splitRegexPipes()` with bracket-depth awareness; replaced 31 occurrences of `regex.split('|')` to correctly handle `|` inside lookbehind/lookahead/groups (e.g., `(?<!拉|欧)美国|United States`) / 新增`_splitRegexPipes()`括号深度感知分割函数，替换31处`regex.split('|')`，修复lookbehind等括号内含`|`时被错误拆分的问题
- **Remove Group Double-Click / 移除分组双击操作** — Removed double-click on style group title and collapsed tab pin-back / 移除双击样式分组标题移动到单显区、双击标签钉回常显的操作
- **Remove PinToNormalDisplay Menu / 移除钉回常显右键选项** — Removed "Pin to normal display" context menu from collapsed category tabs / 移除折叠标签右键菜单中的"钉回常显区"选项

</details>

<details>
<summary>v1.9.9.2 (2026-07-20)</summary>

- **Sidebar Panel Drag Fix / 侧边栏面板拖动修复** — Fixed title bar cursor and drag not being disabled when panel is in sidebar; changed static `_inSidebar` to dynamic `isInSidebar()` check / 修复面板移入侧边栏后标题栏仍显示抓手指针且可拖动的问题，将静态_inSidebar改为动态isInSidebar()检测
- **Keyword Click Group Auto-Switch / 关键词点击分组自动切换** — Clicking a new keyword now automatically switches to the matching rule/style group instead of staying on the manually selected group / 点击新关键词时自动切换到匹配的规则/样式分组，不再停留在手动选择的分组
- **Panel Scroll Position Fix / 面板滚动位置修复** — Removed auto-scroll after showing inline remarks; now preserves scroll position so top chip bar stays visible / 移除备注显示后的自动滚动，改为保存恢复滚动位置，确保顶部chip栏不被滚出视口
...
<details>
<summary>v1.9.9.1 (2026-07-19)</summary>

- **Ungrouped i18n Fix / 未分组国际化修复** — Keywords chip "未分组" now uses i18n function, correctly showing "Ungrouped" in English mode / 关键词chip区域"未分组"改用t()函数，英文模式下正确显示"Ungrouped"
- **Keywords Group Default Name / 关键词分组默认名称** — English default changed from "New Styles" to "new keywords group" / 英文默认名称从"New Styles"改为"new keywords group"
- **CSS Group Default Name / CSS分组默认名称** — CSS chip "+" button presets "new Styles group" as default name / CSS chip"+"按钮预设名称改为"new Styles group"
- **Sidebar SG Icon Auto-Show / 侧边栏SG图标自动显示** — New install auto-shows SG icon in right sidebar / 新装插件自动在右侧面板显示SG图标
- **Main Panel Default Size / 主面板默认尺寸** — Default size changed to 800×500 / 默认尺寸改为800×500
- **Ribbon Icon SG / 左侧功能区图标SG** — Left ribbon icon now displays "SG" text / 左侧功能区图标改为显示"SG"文字

</details>

<details>
<summary>v1.9.9 (2026-07-19)</summary>

- **Floating Note Mobile Drag / 悬浮笔记手机拖动** — Added touch event handlers to floating notes, enabling drag-to-move on mobile devices / 为悬浮笔记添加触摸事件处理，手机端可拖动位置
- **Remark Buttons Size Reduction / 备注按钮缩小** — Reduced remark popup action buttons (?、+、⬡) diameter from 36px to 26px on desktop / 桌面版备注弹窗操作按钮直径从36px减至26px

</details>

<details>
<summary>v1.9.8.4 (2026-07-19)</summary>

- **Popout Note Ctrl+Scroll Opacity Fix / 独立窗口Ctrl+滚轮透明度修复** — Fixed ipc-message args parsing and note.id type mismatch that prevented Ctrl+scroll from adjusting popout window opacity / 修复ipc-message参数解析偏移和note.id类型不匹配导致Ctrl+滚轮无法调整独立窗口透明度
- **Popout Note Restore on Startup / 启动恢复独立窗口修复** — Popout windows now correctly restore on Obsidian startup; fixed `@electron/remote` require order and render timing / 启动时独立窗口正确恢复，修复@electron/remote获取顺序和渲染时序
- **Popout Note Empty Lines Fix / 独立窗口空行压缩修复** — Changed `textContent` to `innerText` in popout blur handler to preserve line breaks / 用innerText替代textContent保留换行符
- **Chip-to-Popout Sync Fix / Chip编辑同步到独立窗口修复** — Editing a note in the chip panel now correctly syncs to the popout window / chip面板编辑笔记现在正确同步到独立窗口
- **Popout Note Frosted Glass Style / 独立窗口毛玻璃样式** — Popout windows now use semi-transparent dark background simulating frosted glass effect / 独立窗口使用半透明深色背景模拟毛玻璃效果
- **Popout Note Theme Switching / 独立窗口配色切换** — Shift+scroll cycles through 5 color themes (dark, light, green, purple, brown) / Shift+滚轮循环切换5套配色（深色、浅色、绿调、紫调、棕调）
- **Popout Note Font Size Zoom / 独立窗口文本缩放** — Alt+scroll adjusts popout note text size (9-28px) / Alt+滚轮调整独立窗口文本字号（9-28px）
- **Startup Performance Optimization / 启动性能优化** — Pinyin/interlinear data deferred; CSS reads unified (eliminated 3x redundancy); CSS parsing merged; rules migration save deferred / 拼音/行间注释延迟加载；CSS读取统一（消除3次冗余）；CSS解析合并；规则迁移回写延迟

</details>

<details>
<summary>v1.9.8.3 (2026-07-18)</summary>

- **Rule Group Buttons Visibility Fix / 规则分组按钮可见性修复** — All buttons in the user's active rule group now remain visible when moving the mouse downward / 手动切换规则分组后鼠标下移时该分组内所有按钮保持可见
- **Rule Group Auto-Activate Guard / 规则分组自动激活守卫** — Auto-activate logic now skips when user has manually selected a different group / 自动激活逻辑在用户已手动选择其他分组时跳过
- **Rule Group Chip Hover Cooldown / 规则分组Chip悬浮冷却** — Added cooldown after rule group chip hover-triggered activation / 规则分组chip悬浮触发激活后添加冷却期

</details>

<details>
<summary>v1.9.8.2 (2026-07-18)</summary>

- **Style Group Switch Fix / 样式分组切换修复** — Switching to a different style group via hover no longer jumps back to the original group / 悬浮切换到其他样式分组时不再跳回原分组
- **Group Chip Hover Cooldown / 分组Chip悬浮冷却** — Added cooldown after group chip hover-triggered click to prevent cascading group toggles / 分组chip悬浮触发click后添加冷却期，防止连锁切换分组

</details>

<details>
<summary>v1.9.8.1 (2026-07-18)</summary>

- **Mobile Chip Toggle Fix / 手机端Chip切换修复** — Tapping an expanded top chip now correctly collapses it on mobile / 手机端点击已展开的顶部chip现在能正确收起
- **No-Remark Keyword Window Fix / 无备注关键词窗口修复** — Keywords without remarks now correctly open the keyword detail window showing related notes and highlights / 无备注关键词现在能正确打开关键词详情窗口显示相关笔记和高亮
- **Related Links Click Fix / 相关链接点击修复** — Related notes and highlights links now work on mobile; fixed `innerHTML +=` destroying event listeners when no remarks exist / 相关笔记和高亮链接在手机端现在可正常点击，修复无备注时事件监听器被销毁的问题

</details>

<details>
<summary>v1.9.8 (2026-07-18)</summary>

- **Panel Chip Bar Position Fix / 面板ChipBar位置修复** — Fixed rule group chips moving below remark section after updating rules / 修复更新规则后规则分组chips移到备注下方的问题
- **Rule Click Input Restore / 规则点击输入框恢复** — After panel auto-refresh, clicking a rule button now correctly fills the regex into the input box / 面板自动刷新后点击规则按钮正确填入关键词到输入框
- **Local Chip Name Translation Fix / 文件规则Chip名称修复** — Fixed file rule chip showing raw translation key instead of "当前文件" / 修复文件规则chip显示未解析翻译键的问题
- **File Rules Load on First Open / 首次打开加载文件规则** — Right panel now loads current file rules on first open, file-name chip group appears immediately / 右侧面板首次打开时加载当前文件规则，文件名chip分组立即可见
- **Collapse Setting Persistence / 折叠设置持久化修复** — Activating a rule group chip no longer overwrites the "默认折叠规则分组" setting / 激活规则分组chip不再覆盖"默认折叠规则分组"设置
- **Chip Hover After Rule Delete / 删除规则后Chip悬浮修复** — Top chip bar hover behavior no longer breaks after deleting rules / 删除规则后顶部chip悬浮触发不再失效
- **Rule Group Activation Fix / 规则分组激活修复** — Clicking a global rule button no longer jumps to the file-rules group / 点击全局规则按钮不再跳到文件规则分组

</details>

<details>
<summary>v1.9.7 (2026-07-18)</summary>
...
- **clearHighlights Memory Fix / clearHighlights内存修复** — Replaced array accumulation with DocumentFragment to prevent RangeError on large documents / 用DocumentFragment替代数组累积，防止大文档RangeError
...
<details>
<summary>v1.9.6.2 (2026-07-18)</summary>
...
<details>
<summary>v1.9.6.1 (2026-07-17)</summary>

- **Related Notes Middle-Click Fix / 相关笔记中键点击修复** — Middle-click on related note links now opens in background tab; blocked browser auto-scroll on middle-click / 相关笔记链接中键点击改为后台新标签页打开，阻止默认自动滚动
- **Related Notes Expand All / 相关笔记展开全部** — The "+n" tag is now clickable; clicking expands all remaining note links / "+n"标签可点击，点击后展开所有剩余链接

</details>

<details>
<summary>v1.9.6 (2026-07-17)</summary>

- **Panel Auto-Refresh / 面板自动刷新** — Right panel listens to `rulesUpdateEmitter`; clicking l/g button or adding rules via floating ball instantly updates the panel / 右侧面板监听规则更新事件，点击l/g按钮或悬浮球添加规则后面板即时刷新
- **Mobile Sidebar Remark / 手机端侧边栏备注** — New "Mobile Remark in Sidebar" setting; clicking keyword shows remarks in sidebar background with haptic feedback, swipe to view / 新增"手机端备注在侧边栏显示"设置，点击关键词在侧边栏后台显示备注，带震动反馈
- **Mobile Sidebar Fix / 手机端侧边栏修复** — Fixed long-press/select text incorrectly closing sidebar panel on mobile / 修复手机端长按选中文本误关闭侧边栏面板
- **Remark Text Selectable / 备注文字可选** — Remark area now supports text selection and drag-to-search / 备注区域支持文字选中拖放搜索

</details>

<details>
<summary>v1.9.5 (2026-07-16)</summary>

- **Related Notes / 显示相关笔记** — New "Show Related Notes" setting; displays notes with matching filenames above remark area on keyword select/click; left-click opens new tab, middle-click opens background tab; works for non-rule text too / 新增"显示相关笔记"设置，选中/点击关键词时在备注上方显示相关笔记链接，左键新标签页打开，中键后台打开，非规则文本也支持
- **Underline Count Support / 下划线样式计数支持** — Removed 3 exclusion points; underline-style keywords now display match counts normally / 移除3处排除逻辑，下划线样式关键词可正常显示计数
- **Remark Add Button Fix / 备注添加按钮修复** — Rewrote "+" button save logic using `ruleSource` (matching n-button pattern); fixed first-time remark addition failing for local rules in sidebar / 重写"+"按钮保存逻辑，修复侧边栏局部规则首次添加备注失败
- **Sidebar Panel Group Collapse / 侧边栏分组折叠** — All style groups collapsed by default when no keyword matches, reducing visual clutter / 无关键词匹配时所有样式分组默认折叠
- **Sidebar Button Hide / 侧边栏按钮隐藏** — "Move to Sidebar" and "Close" buttons hidden in sidebar mode; no longer reappear after language switch / 侧边栏模式隐藏按钮，切换语言后不再闪现
- **Remark Filename Edit / 备注文件名编辑** — Double-click file name tag to edit filePath; Enter saves, Escape cancels / 双击文件名标签可编辑filePath
- **AI Button Shape / AI按钮圆形化** — AI Ask, AI Graph, Add (+) buttons now 36px circles on desktop / AI提问、AI关系图、添加按钮桌面端统一为36px圆形
- **Status Bar Hide / 状态栏隐藏** — Settings popup uses `display:none` to hide status bar / 设置弹窗改用`display:none`隐藏状态栏
- **Debug Logs Removed / 调试日志移除** — Removed all 14 `[SG-DEBUG]` console.log statements / 移除全部14条调试日志

</details>

<details>
<summary>v1.9.4.3 (2026-07-15)</summary>

- **Popup Mouse Tracking Fix / 弹窗鼠标追踪修复** — Replaced unreliable `relatedTarget` detection with `mousemove` bounds-box; added `popup.contains` check for fixed badges / 将不可靠的`relatedTarget`改为`mousemove`包围盒检测，增加`popup.contains`覆盖fixed定位badge
- **Dynamic Z-Index / 动态Z层级** — Added `bringToFront` with `_topZ` counter; all popups raise z-index on mousedown, fixing chip preview behind windows / 新增`bringToFront`动态提升z-index，修复chip预览显示在窗口下方
- **AI Graph Height Fix / AI关系图高度修复** — `applyZoom` sets SVG dimensions directly; `tryFit` waits for actual content with `requestAnimationFrame`; viewport height synced / 直接设SVG宽高，等待实际图形元素渲染，viewport高度同步
- **SVG className Fix / SVG className修复** — Fixed crash on SVG `className` (SVGAnimatedString) / 修复SVG元素className类型错误崩溃
- **Scroll Position Preserve / 滚动位置保留** — Remark content re-render preserves scroll position / 备注内容重渲染保留滚动位置
- **Image Hover Skip / 图片悬浮预览跳过** — No hover preview when image already at ≥95% natural size / 图片已100%显示时不再弹出悬浮预览
- **Debug Logs Removed / 调试日志移除** — Removed all `[RemarkPopup]` console logs / 移除所有调试日志

</details>

<details>
<summary>v1.9.4.2 (2026-07-14)</summary>

- **AI Graph Async Render Fix / AI关系图异步渲染修复** — Fixed remark popup showing incomplete AI relationship graph on open; added MutationObserver to detect SVG appearance and re-apply zoom, plus `onReposition` callback to recalculate popup position after graph is fully rendered / 修复备注弹窗打开时AI关系图显示不全的问题；新增MutationObserver监听SVG出现后重新应用缩放，并添加onReposition回调在图形渲染完成后重新计算弹窗位置

</details>

<details>
<summary>v1.9.4.1 (2026-07-14)</summary>

- **Interlinear Note Regex Key Fix / 行间注释正则Key修复** — Fixed interlinear note using matched text as key instead of full regex; remark popup now uses ruleRegex as key / 修复行间注释以匹配文本而非完整正则作为key的问题，备注弹窗现在使用ruleRegex作为key
- **Interlinear Note Regex Rendering / 行间注释正则渲染** — Rendering now splits regex keys by `|` to match each alternative separately / 渲染时按`|`分割正则key分别匹配，支持多分支正则模式
- **Interlinear Note Auto-Migration / 行间注释自动迁移** — When rule regex changes, old key is automatically found and migrated to new key / 规则正则变更后自动查找旧key并迁移到新key
- **List Mode Title Size Fix / 列表模式标题大小修复** — Adjusted remark popup list mode file title from 12px to 10px to match masonry mode / 备注弹窗列表模式文档标题从12px调整为与瀑布流一致的10px

</details>

<details>
<summary>v1.9.4 (2026-07-12)</summary>

- **Mermaid Parser Rewrite / Mermaid解析器重写** — Rewrote `_sanitizeMermaid` line parser with cross-line node tracking and bracket definition helpers; correctly handles chained edges and bracket node definitions / 重写行解析器，新增跨行节点追踪和辅助函数，正确处理链式边和带括号定义的节点
- **AI Graph Anti-Weak-Relation / AI关系图防牵强关系** — Added context snippets for related keywords in prompt; new rule forbids creating edges for incidental word usage / 新增关联依据上下文片段，严禁为偶然用词建边
- **AI Graph Bilingual Prompt / AI关系图双语Prompt** — Auto-detects remark language and selects Chinese or English prompt accordingly; Japanese/Korean use English prompt / 自动检测备注语言选择对应Prompt，日语韩语用英文Prompt
- **Graph Zoom Height Fix / 关系图缩放高度修复** — Zoom height now calculated from SVG viewBox on initial render, no manual zoom needed / 缩放高度改用viewBox计算，初始渲染即正确适配
- **Graph Scrollbar Fix / 关系图滚动条修复** — Removed unnecessary vertical scrollbar from graph viewport / 移除关系图区域不必要的垂直滚动条
- **Masonry Mode i18n / 瀑布流模式国际化** — Replaced hardcoded Chinese text with i18n key for masonry mode tooltip / 瀑布流模式提示文字改为i18n调用

</details>

<details>
<summary>v1.9.3.4 (2026-07-11)</summary>

- **Random Group All Styles / 随机分组全部样式** — Fixed floating ball random highlight with group filter showing "no unused styles"; now picks from all styles in the group / 修复悬浮球随机分组提示"没有可用的未使用样式"，现在从分组内所有样式中随机选取
- **Open Doc Button Style / 打开文档按钮样式** — Changed open document button icon to document+🔍 matching SwiftMatch / 打开文档按钮图标改为与SwiftMatch一致的文档+🔍样式
- **Paste Remark Button / 粘贴备注按钮** — Added paste button between open document and copy buttons; appends to existing remark / 在打开文档与复制按钮间新增粘贴按钮，已有内容时追加
- **Paste in Edit Mode / 编辑状态粘贴修复** — Fixed paste button losing textarea focus; now inserts at cursor position / 修复编辑态粘贴导致失焦，现在在光标位置插入

</details>

<details>
<summary>v1.9.3.3 (2026-07-09)</summary>

- **Remark Badge Wrong Target Fix / 备注n按钮错位修复** — Fixed clicking "n" badge adding remark to wrong popup when another keyword's popup is already open / 修复点击"n"按钮添加备注时，若其他关键词弹窗已打开，备注会错误添加到已打开弹窗
- **Multiple Remark Popups / 多备注弹窗共存** — Different keywords' remark popups can stay open simultaneously / 不同关键词的备注弹窗可同时打开

</details>

<details>
<summary>v1.9.3.2 (2026-07-09)</summary>

- **Floating Group Middle-Click Global / 悬浮分组随机模式中键全局** — Middle-click floating group button with random mode applies as global rule, same as floating ball / 开启随机模式的悬浮分组按钮中键点击应用为全局规则，与悬浮球一致
- **Mobile Main Panel Drag / 手机端主面板拖动** — Touch drag support on main panel title bar for mobile / 主面板标题栏添加触摸拖动支持
- **Shortcut Delete Global Rule / 快捷键删除全局规则** — Remove style shortcut now deletes global rules too; confirmation when rule has remarks / 移除样式快捷键支持删除全局规则，有备注时确认
- **Remark Popup Long-Press Close Fix / 备注弹窗长按关闭修复** — Fixed long-press close preventing subsequent remark popups / 修复长按关闭后备注弹窗不再触发

</details>

<details>
<summary>v1.9.3.1 (2026-07-09)</summary>

- **Rename to SwiftGlossa / 更名为SwiftGlossa** — Renamed from SwiftGloss to SwiftGlossa; plugin ID unchanged / 从SwiftGloss更名为SwiftGlossa；插件ID不变
